// Type: System.ServiceProcess.NativeMethods
...
namespace System.ServiceProcess
{
  internal static class NativeMethods
  {
    ...
    [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static IntPtr RegisterServiceCtrlHandler(string serviceName, Delegate callback);

    [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static IntPtr RegisterServiceCtrlHandlerEx(string serviceName, Delegate callback, IntPtr userData);

    [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static bool SetServiceStatus(IntPtr serviceStatusHandle, System.ServiceProcess.NativeMethods.SERVICE_STATUS* status);

    [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static bool StartServiceCtrlDispatcher(IntPtr entry);
    ...
    [DllImport("advapi32.dll", CharSet = CharSet.Unicode)]
    public static int LsaAddAccountRights(IntPtr policyHandle, byte[] accountSid, System.ServiceProcess.NativeMethods.LSA_UNICODE_STRING userRights, int countOfRights);
    ...
    [DllImport("advapi32.dll", CharSet = CharSet.Unicode)]
    public static int LsaEnumerateAccountRights(IntPtr policyHandle, byte[] accountSid, out IntPtr pLsaUnicodeStringUserRights, out int RightsCount);

    [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static bool LookupAccountName(string systemName, string accountName, byte[] sid, int[] sidLen, char[] refDomainName, int[] domNameLen, [In, Out] int[] sidNameUse);

    [DllImport("kernel32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
    public static bool GetComputerName(StringBuilder lpBuffer, ref int nSize);
